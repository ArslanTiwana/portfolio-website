...
export default function ContactSection() {
  const contactMethods = [
    {
      icon: <Github className="w-10 h-10 text-slate-300 group-hover:text-white" />,
      title: "GitHub",
    ...
    {
      icon: <Linkedin className="w-10 h-10 text-blue-400 group-hover:text-blue-300" />,
      title: "LinkedIn",
      description: "Professional network",
      href: "https://linkedin.com/in/ArslanTiwana",
      hoverClass: "group-hover:text-blue-300"
    },
    {
      icon: <Mail className="w-10 h-10 text-emerald-400 group-hover:text-emerald-300" />,
      title: "Email",
      description: "Get in touch",
      href: "mailto:muhammadarslan0111@email.com",
              ...
              <Card key={index} className="bg-slate-800 border-slate-700 hover:bg-slate-700 transition-colors group">
                <CardContent className="p-6">
                  <a href={method.href} target="_blank" rel="noopener noreferrer" className="block">
                    ...
                    <p className="text-slate-400 group-hover:text-slate-300">{method.description}</p>
                  </a>
                </CardContent>
              </Card>
            ))}
